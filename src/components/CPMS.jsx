...
import Tollgate from "../images/car-park-tollgate-cpms.jpg"
import Cpms from "../images/cpms-hilton1.jpg"
import { cpms } from "../data";
import Card from '../UI/Card';
import { Link } from 'react-router-dom';

const CPMS = () => {
  const [index, setIndex] = useState(0);
  const { name, avatar } = cpms[index];

  const prevTestimonialHandler = () => {
    setIndex(prev => prev - 1);
    if (index <= 0) {
      setIndex(cpms.length - 1)
    }
  }
  const nextTestimonialHandler = () => {
    setIndex(prev => prev + 1);
    if (index >= cpms.length - 1) {
      setIndex(0)
    }
  }
  return (
      ...
      <div className="container">
        ...
        <section className="testimonials">
          <div className="testimonials__container">
            <SectionHead icon={<ImQuotesLeft />} title="Parking System" className="testimonials__head" />
